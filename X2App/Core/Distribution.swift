...
protocol Distribution {
    ...
    var degreesOfFreedom: Int { get set }
...
extension Distribution {
    func chiSquaredEmpiric() -> Double {
        ...
        return data.reduce(0.0) { $0 +
            pow(Double($1.dataInfo.amount) - total * $1.probability, 2) / (total * $1.probability) }
    }
    ...
    mutating func regroup() {
        ...
        while index < data.count {
            if data[index].dataInfo.amount < 5 && totalAmount*data[index].probability < 10 { // check, do we need to merge to the right next
        ...
        while data[data.count - 1].dataInfo.amount < 5 && totalAmount*probability(of: data[data.count - 1].dataInfo) < 10 {
